...
(function($){
	...
	var gallerydefaults = {
		...
		curslide: 0,
		threshold: 75,
		navbuttons : ['img/left.png', 'img/right.png', 230]
	}
	...
	$.fn.touchgallery = function(options){
		return this.each(function(){
			...
			var $navbuttons = $('<div style="position:absolute; width:10%; height: 100%; left: 0; top: 0; text-align: left; line-height:' + setting.height + 'px; cursor: pointer; opacity: 0" />')
				.clone()
				.addBack()
				.eq(0)
				.data({'dir': 'back'})
				.html('<img src="' + setting.navbuttons[0] + '" style="vertical-align: middle; position:relative; top:' + setting.navbuttons[2] +'px; left:1em; title="Back one image" />')
				.end()
				.eq(1)
				.data({'dir': 'forth'})
				.css({left: 'auto', right: 0, textAlign: 'right'}).html('<img src="' + setting.navbuttons[1] + '" style="vertical-align: middle; position:relative; top:' + setting.navbuttons[2] +'px; float: right; right: 1em; title="Forward one image" />')
				.end()
